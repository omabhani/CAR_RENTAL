...
if (isset($_POST['register'])) {
    ...
    if ($password !== $cpassword) {
        ...
        $check = mysqli_query($conn, "SELECT * FROM buyers WHERE email='$email'");
        if (mysqli_num_rows($check) > 0) {
            echo "<script>alert('Email already exists');</script>";
        } else {
            $query = "INSERT INTO buyers (name, email, phone, password) VALUES ('$name', '$email', '$phone', '$hashedPassword')";
            if (mysqli_query($conn, $query)) {
                echo "<script>alert('Registration successful!'); window.location.href='buyer_login.php';</script>";
            } else {
...
<nav class="navbar">
  ...
  <div class="form-container">
    ...
    <form method="post" action="">
      ...
      <p>Already have an account? <a href="buyer_login.php">Login</a></p>
    </form>
  </div>
</body>
</html>
