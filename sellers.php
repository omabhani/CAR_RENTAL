...
$count_query = "SELECT COUNT(*) as total FROM sellers" . $where_clause;
$count_result = mysqli_query($conn, $count_query);
$total_sellers = mysqli_fetch_assoc($count_result)['total'];
$total_pages = ceil($total_sellers / $per_page);

// Get sellers with cars count and revenue
$query = "SELECT s.*, 
          (SELECT COUNT(*) FROM cars WHERE seller_id = s.id) as cars_count,
          (SELECT SUM(b.total_amount) FROM bookings b 
           JOIN cars c ON b.car_id = c.id 
           WHERE c.seller_id = s.id) as total_revenue
          FROM sellers s" . $where_clause . 
...
if (isset($_POST['delete_seller'])) {
    ...
    // Check if there are any cars for this seller
    $check_cars = mysqli_query($conn, "SELECT COUNT(*) as count FROM cars WHERE seller_id = $seller_id");
    $has_cars = mysqli_fetch_assoc($check_cars)['count'] > 0;
    
    if ($has_cars) {
        $delete_error = "Cannot delete seller with existing cars. Please delete or reassign their cars first.";
    } else {
        mysqli_query($conn, "DELETE FROM sellers WHERE id = $seller_id");
        header("Location: sellers.php?deleted=true");
        exit();
    }
}
?>
...
<head>
    ...
    <title>Sellers - CarRental Admin</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="alert alert-success">
            New seller has been added successfully.
        </div>
        <?php endif; ?>
        
        <?php if(isset($_GET['updated']) && $_GET['updated'] == 'true'): ?>
        <div class="alert alert-success">
            Seller information has been updated successfully.
        </div>
        <?php endif; ?>
        ...
        <div class="card">
            <div class="stats-row">
                <div class="stat-item">
                    ...
                    <span class="value"><?php echo $total_sellers; ?></span>
                </div>
                <div class="stat-item">
                    <span class="label">Total Cars Listed</span>
                    <span class="value">
                        <?php 
                        $total_cars = mysqli_fetch_assoc(mysqli_query($conn, 
                            "SELECT COUNT(*) as count FROM cars"))['count'];
                        echo $total_cars;
                        ?>
                    </span>
                </div>
                <div class="stat-item">
                    <span class="label">Total Revenue Generated</span>
                    <span class="value">
                        <?php 
                        $total_revenue = mysqli_fetch_assoc(mysqli_query($conn, 
                            "SELECT SUM(b.total_amount) as total FROM bookings b JOIN cars c ON b.car_id = c.id"))['total'] ?? 0;
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th width="10%">Cars Listed</th>
                            <th width="15%">Revenue Generated</th>
                            
                        </tr>
                    </thead>
                    <tbody>
                        <?php if(mysqli_num_rows($sellers) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge badge-primary">
                                        <?php echo $seller['cars_count']; ?> cars
                                    </span>
                                </td>
    ...
    <div id="deleteModal" class="modal">
        <div class="modal-content">
            <h2>Delete Seller</h2>
            <p>Are you sure you want to delete <strong id="sellerName"></strong>? This action cannot be undone.</p>
